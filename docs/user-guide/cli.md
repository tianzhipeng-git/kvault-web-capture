...
站点配置和标签定义均使用 JSON 文件。Tag 字段、WebUI 操作和规则联动见 [项目 Tag（分类标签）配置指南](./project-label-configuration.md)；站点配置格式见 [规则格式编写指南](./site-config-rule-format-guide.md)、[抓取 Profile 配置](./site-config-capture-profile.md)、[截图与多变体配置](./site-config-screenshot.md) 和 [抓取结果校验](./site-config-validation.md)。
