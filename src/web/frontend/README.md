...
3. **站点控制台 (Site Dashboard)**
   - 针对单一站点的操作和监控中心，分为四个主要模块：
     ...
     - **规则配置 (Config)**：编辑该站点的抓取规则（包含 URL 黑白名单、Tag 规则等）。
     - **采集流程 (Pipeline)**：使用流程图/向导的模式，引导用户按步骤完成 `配置规则 -> 初步摸底 (Seed Run) -> 人工复核 -> 正式采集 (Crawl Run)` 的全生命周期。
     - **页面清单 (Pages)**：呈现被发现的所有页面及其业务状态（如 `stage2_captured`）。用户可点击单行查看该页面的具体底层运行记录（如 Base、Markdown 和 Screenshot 状态）。
