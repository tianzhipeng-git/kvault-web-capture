...
import {
  extractLabelDefinitionCores,
  labelCoresToJsonl,
} from '../src/classification/label-definitions.js';
...
describe('label definitions', () => {
  it('extracts classifier core fields from exported label definitions', () => {
    ...
    expect(extractLabelDefinitionCores(exported)).toEqual([
      {
        key: 'webpage_category_main',
        name: '网页类别-大类',
        description: '页面分类',
    ...
    expect(labelCoresToJsonl(exported)).toBe(
